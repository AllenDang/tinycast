...
enum CalcTokenizer {
            ...
            if isDigit(ch) || (ch == "." && i + 1 < chars.count && isDigit(chars[i + 1])) {
                ...
                while i < chars.count {
                    ...
                    if isDigit(c) {
                        ...
                        // Grouping separator only when followed by exactly 3 digits then a non-digit —
                        // otherwise it's a vector comma (e.g. "1,0,0" stays three tokens).
    ...
    /// True when the comma at `index` is a thousands grouping separator: followed by exactly 3 digits then a non-digit.
...
enum CalcParser {
    /// Evaluate to a scalar Double. Vector results return nil (use `evaluateValue` for full CalcValue support).
...
private struct Parser {
    ...
    /// Parse a parenthesized group: either a scalar expression `(1+2)` or a vector literal `(1, 2, 3)`.
