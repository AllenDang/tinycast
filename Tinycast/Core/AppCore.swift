...
final class AppCore {
    ...
    func showPalette(mode: PaletteMode, restoreAnyMode: Bool = false) {
        ...
        // AI provider config reads the keychain — cache it once per palette open, not on every keystroke.
    ...
    // MARK: - Dialogs
    // Routed through `AppCore` so `dialogs` stays the single owner; flows outside the palette
    // (the backup actions) reach the same dialogs instead of falling back to an `NSAlert`.
