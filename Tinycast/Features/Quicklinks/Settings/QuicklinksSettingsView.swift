...
struct QuicklinksSettingsView: View {
    ...
    var body: some View {
        ...
        .sheet(item: $coordinator.pendingEdit) { request in
    ...
    private var library: some View {
        ...
        SettingsCard {
            if results.isEmpty {
                ...
                ForEach(Array(results.enumerated()), id: \.element.id) { index, quicklink in
                    ...
                    QuicklinkSettingsRow(
                        ...
                        onDelete: {
                            Task { await coordinator.deleteQuicklinkFromSettings(id: quicklink.id) }
                        })
                }
            }
            SettingsDivider()
            ...
            SettingsRow(
                ...
                Button("Add…") { coordinator.editQuicklink(nil) }
                    .controlSize(.small)
            }
        }
    }
    ...
    private var transfer: some View {
        SettingsCard(header: "Import & Export") {
            SettingsRow(
                ...
                Button("Import…") {
                    Task { await coordinator.importQuicklinks() }
                }
                    .controlSize(.small)
            }
            SettingsDivider()
            SettingsRow(
                ...
                Button("Export…") {
                    Task { await coordinator.exportQuicklinks() }
                }
                    .controlSize(.small)
