...
final class SnippetTextInjector {
    ...
    private let clipboardManager: ClipboardMonitor
    ...
    init(clipboardManager: ClipboardMonitor, settings: AppSettings) {
    ...
    func cancelArgumentPrompt(
        ...
        if let automaticGeneration {
            guard Self.argumentPromptCanCancel(
                requestedGeneration: automaticGeneration,
                currentGeneration: self.automaticGeneration)
            else { return }
    ...
    nonisolated static func argumentPromptCanCancel(
        requestedGeneration: AutomaticGeneration,
        currentGeneration: AutomaticGeneration
    ) -> Bool {
        ...
        for (index, item) in items.enumerated() {
            ...
            if index == 0, let firstString {
                guard pasteboardItem.setString(firstString, forType: .string),
                    pasteboardItem.setData(Data(), forType: ClipboardMonitor.internalType)
                else { return nil }
            }
