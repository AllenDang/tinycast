...
struct EmojiScreen: PaletteScreen {
    ...
    func actions(at selection: Int) -> PopoverMenuContent? {
        ...
        return EmojiActionsMenu.content(
            entry: entry, coordinator: coordinator, target: vm.pasteTarget)
    }
    ...
    func perform(_ command: PaletteCommand, at selection: Int) -> Bool {
        guard command == .alternateActivate, let entry = entry(at: selection) else { return false }
        coordinator.pasteKeepingWindowOpen(entry)
        return true
    }
...
enum EmojiActionsMenu {
    static func content(
        entry: EmojiEntry, coordinator: EmojiCoordinator, target: PasteTarget?
    ) -> PopoverMenuContent {
        PopoverMenuContent(
