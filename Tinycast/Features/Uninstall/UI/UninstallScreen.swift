...
struct UninstallScreen: PaletteScreen {
    ...
    func actions(at selection: Int) -> PopoverMenuContent? {
        ...
        return UninstallActionsMenu.content(
            candidate: candidate, session: session, coordinator: coordinator)
    }

    /// The primary action trashes the session's checked set, not the highlighted row.
    func activate(at selection: Int) {
        coordinator.performUninstall()
    }
...
enum UninstallActionsMenu {
    static func content(
        candidate: UninstallCandidate, session: UninstallSession,
