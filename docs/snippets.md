...
| `{input}`                                   | The text typed after an [AI command](ai-commands.md)'s keyword. Empty for every other caller — it exists so an AI command's prompt template doesn't have to repurpose `{selection}` (a captured *app* selection) or `{argument}` (which always prompts when missing) |
