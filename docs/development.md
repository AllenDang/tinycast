...
to the visible row order, which is why that file must stay Foundation-only while it lives under
`Palette/`. It cannot import SwiftUI, so it tests no `body` — instead it asserts the row-order
